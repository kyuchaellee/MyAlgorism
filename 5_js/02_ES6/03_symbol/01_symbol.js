//Symbol 외부코드에서 접근 불가능, 값도 덮어 쓸 수 없는 숨김 프로퍼티를 만들 수 있다.
...
//Symbol.for(key)를 사용해 심볼을 전역 심볼 레지스트리에 만들고 읽는다. 그래서 동일한 이름으로 존재하지 않으면 새로운 심볼 만들고 있다면 심볼을 다시 읽어온다.
let symbol = Symbol.for("id1");
let id1 = Symbol.for("id1");
console.log(symbol === id1);

// 이름이 같더라도 값이 항상 다르므로 이름이 같을 때 값도 같길 원한다면 전역 레지스트에 생성해서 사용해야 됨.