package com.uni.section7.kindofvariable;
...
public class KindOfVariable {
	...
	/* 클래스 영역
	   ...
	   public void testMethod(int num) {// 매개 변수 : 지역 변수
		   // 메소드영역에서 작성하는 변수: 지역 변수
		   int localNum;
		   
		   System.out.println(num); //매개 변수는 호출시 값이 넘어와서 변경되기 때문에 초기화가 필요 없다.
//		   System.out.println(localNum); // 지역 변수는 초기화가 반드시 되야 한다.
...
//		   System.out.println(localNum); 지역 변수는 해당 블록 안에서만 사용 가능.
		   System.out.println(globalNum);
		   System.out.println(staticNum);
	   }
}
...
//public class Member {} // 퍼블릭은 1 개만 가능.
class Member {} //다른 패키지 외부에서 접근 불가.
